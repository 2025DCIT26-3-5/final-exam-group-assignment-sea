...
export function MapboxFriendTrackingWithBackend() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.friends && data.friends.length > 0) {
          // Merge backend data with local friends
          const backendFriends = data.friends.map((f: any) => {
            const localFriend = friends.find(
              (lf) => lf.id === f.friendId,
            );
            return {
              id: f.friendId,
              name: localFriend?.name || `Friend ${f.friendId}`,
              ...
              avatar:
                localFriend?.avatar ||
                f.friendId.substring(0, 2).toUpperCase(),
              status: localFriend?.status || "online",
              speed: f.speed || 0,
              heading: f.heading || 0,
            };
          });
      ...
      console.error(
        "Error fetching friends from backend:",
        error,
      );
    }
    return null;
  ...
  // Sync local friends to backend
  const syncFriendsToBackend = async () => {
    if (!backendConnected) return;

    try {
      for (const friend of friends) {
        await fetch(`${serverUrl}/friends/location`, {
          method: "POST",
          headers: {
      ...
      console.error("Error syncing friends to backend:", error);
    }
  };
  ...
  const updateAllWeather = async () => {
    ...
    const updatedFriends = await Promise.all(
      friends.map(async (friend) => {
        ...
        // Sync weather to backend if connected
        if (backendConnected) {
          try {
            await fetch(`${serverUrl}/friends/weather`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${publicAnonKey}`,
            ...
            console.error(
              "Error syncing weather to backend:",
              error,
            );
          }
        }
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (connected) {
        ...
        const backendFriends = await fetchFriendsFromBackend();
    ...
    // Check for friend location updates every 30 seconds if backend is connected
    ...
    <div className="space-y-4 p-4">
      ...
      {!backendConnected && (
        <Alert>
          ...
          <AlertDescription>
            Using local data. Enable Supabase backend for
          ...
          <div className="flex items-start space-x-2">
            ...
            <div>
              <p className="text-sm text-blue-800">
                {backendConnected
                  ? "Real-time Weather & Location Sync Enabled"
                  : "Real-time Weather Updates Active"}
              </p>
              <p className="text-xs text-blue-600 mt-1">
                {backendConnected
                  ? "Friend locations and weather sync across all devices in real-time. Weather layers show temperature overlays."
                  : "Weather conditions update automatically. Connect to Supabase backend for real-time location sharing across devices."}
              </p>
            </div>
          </div>
        </CardContent>
